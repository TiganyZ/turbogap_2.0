...
module vdw_interface
   use kinds, only: dp
   use vdw_types, only: vdw_t
   use local_prop, only: lp_indexes_t
   use types, only: state_t, calculation_t, neighbors_t, split_t, species_info_t
   use control, only: control_t
   use timing, only: time_start, time_end

contains

   subroutine get_vdw_energy_and_forces(state, species_info, this_vdw_calculation, vdw_calculation, options_vdw, &
                                        neighbors, local_properties_cart_der, lp_indexes, split, do_, &
                                        time_vdw)
      type(state_t), intent(in) :: state
      type(species_info_t), intent(in) :: species_info
      type(calculation_t), intent(inout) :: this_vdw_calculation
      type(calculation_t), intent(inout) :: vdw_calculation
      type(neighbors_t), intent(in) :: neighbors
      type(split_t), intent(in) :: split
      real(dp), allocatable, intent(inout) :: local_properties_cart_der(:, :, :)
      real(dp), allocatable :: v_neigh_vdw(:)
      type(vdw_t), intent(inout) :: options_vdw
      type(lp_indexes_t), intent(in) :: lp_indexes
      type(control_t), intent(inout) :: do_
      real(dp), intent(inout) :: time_vdw(3)
      integer :: i
      integer :: j
      integer :: k
      integer :: j2
      ...
      call time_start(time_vdw)

      if (options_vdw%type == "ts" .or. options_vdw%type == "TS") then

         allocate (v_neigh_vdw(1:split%j_end - split%j_beg + 1), source=0.0_dp)
         v_neigh_vdw = 0.d0
         k = 0
         ...
         call get_ts_energy_and_forces(state%local_properties(split%i_beg:split%i_end, lp_indexes%hirshfeld_v), & !hirshfeld_v, &
                                       ...
                                       options_vdw%c6_ref, &
                                       options_vdw%r0_ref, &
                                       options_vdw%alpha0_ref, &
                                       do_%forces, &
                                       ! #ifdef _MPIF90
                                       !                                        this_vdw_calculation%energies, &
...
!#endif
      ...
      call time_end(time_vdw)

   end subroutine get_vdw_energy_and_forces

   !**************************************************************************
   subroutine get_ts_energy_and_forces(hirshfeld_v, hirshfeld_v_cart_der, &
                                       n_sites, n_pairs, n_species, n_sites0, &
      ...
      integer, intent(in) :: n_sites, n_pairs, n_species, n_sites0
      ...
      do i = 1, n_sites
         ...
         do j = 2, n_neigh(i)
            ...
            if (rjs(k) > rcut_inner .and. rjs(k) < rcut) then
               ...
               ! print *, size(energies), i
               ! print *, size(neighbor_c6_ij), k
               ! print *, size(r6), k
               ! print *, size(f_damp), k
   ...
   end subroutine get_ts_energy_and_forces
   !**************************************************************************
   ...
   end subroutine get_vdw_ref_params
   !**************************************************************************

end module vdw_interface
