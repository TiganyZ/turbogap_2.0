...
! HND X
! HND X   This file, gap_interface.f90, is copyright (c) 2019-2025, Miguel A. Caro and
...
!   and on which forces might be acting
      ...
      n_atom_pairs = 0
      k = 0
      allocate (is_atom_seen(1:n_total_sites), source=.false.)
...
!     Get energies and forces
         allocate (energies(1:n_sites), source=0.0_dp)
         if (do_forces) then
            allocate (forces(1:3, 1:n_all_sites), source=0.0_dp)
         end if
         if (n_sites > 0) then
            call get_soap_energy_and_forces(soap, soap_cart_der, alphas, delta, zeta, 0._dp, Qs, &
