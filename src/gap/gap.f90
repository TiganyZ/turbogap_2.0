...
! HND X
! HND X   This file, gap.f90, is copyright (c) 2019-2025, Miguel A. Caro and
...
!   Energies
      ...
      allocate (kernels(1:n_sites, 1:n_sparse), source=0.0_dp)
      allocate (kernels_copy(1:n_sites, 1:n_sparse))
      if (n_sites > 0) then
         call dgemm("t", "n", n_sites, n_sparse, n_soap, 1._dp, soap, n_soap, Qs, n_soap, 0._dp, &
                    kernels, n_sites)
      end if
...
!   Forces
      if (do_forces) then
         ...
         if (is_zeta_int) then
            ...
            kernels_der = kernels**(zeta - 1._dp)
         end if
         ...
         if (n_sites > 0) then
            call dgemm("n", "t", n_sites, n_soap, n_sparse, -zeta*delta**2, kernels_der, n_sites, &
                       Qs_copy, n_soap, 0._dp, Qss, n_sites)
         end if
...
!          virial = virial + dot_product( this_force(1:3), xyz(1:3,l) )
               do k1 = 1, 3
                  do k2 = 1, 3
                     virial(k1, k2) = virial(k1, k2) + 0.5_dp*(this_force(k1)*xyz(k2, l) + this_force(k2)*xyz(k1, l))
                  end do
               end do
            end do
         end do
         ...
         do j = 2, n_neigh(i)
            ...
            if (rjs(k) < rcut) then
               if (rjs(k) < rcut - buffer) then
                  ...
                  fcut = (dcos(pi*(rjs(k) - rcut + buffer)/buffer) + 1._dp)/2._dp
               end if
               do s = 1, n_sparse
                  energies(i) = energies(i) + delta**2*alphas(s)*cutoff(s)*fcut* &
                                dexp(-0.5_dp*(rjs(k) - Qs(s))**2/sigma**2)
               end do
            end if
         end do
      end do
            ...
            do j = 2, n_neigh(i)
               ...
               if (rjs(k) < rcut) then
                  if (rjs(k) < rcut - buffer) then
                     ...
                     fcut = (dcos(pi*(rjs(k) - rcut + buffer)/buffer) + 1._dp)/2._dp
                     dfcut = pi/2._dp/buffer*dsin(pi*(rjs(k) - rcut + buffer)/buffer)
                  end if
                  do s = 1, n_sparse
                     this_force(1:3) = -2._dp*delta**2*alphas(s)*cutoff(s)* &
                                       dexp(-0.5_dp*(rjs(k) - Qs(s))**2/sigma**2)* &
      ...
      do i = 1, n_sites
         ...
         do j = 2, n_neigh(i)
            ...
            if (rjs(k) < rcut) then
               ...
               energies(i) = energies(i) + 0.5_dp*V_int(1)
            end if
         end do
      end do
...
!
   ...
   subroutine calculate_gap_3b(n_sites0, neighbors, n_gap_3b, gap_3b_hypers, do_, &
   ...
   subroutine get_3b_energy_and_forces(n_sites0, rjs, xyz, alphas, cutoff, rcut, buffer, delta, sigma, e0, Qs, &
...
!   Internal variables
      ...
      real(dp), allocatable :: r(:), drdq(:, :), kernel(:), drdx1(:, :), drdx2(:, :), drdx3(:, :), pref(:), &
                               kernel_der(:)
      integer :: n_sparse, i, j, k, k2, n_sites, n_atom_pairs, s, j2, i3, j3, k3, l, sp0, sp1, sp2, k1, k4
...
!   (or maybe not...)
      ...
      pref(1:n_sparse) = 2._dp*delta**2*alphas(1:n_sparse)*cutoff(1:n_sparse)
...
!         reduced the calculation time by about 40% for the large random C system with a 3 Angstrom cutoff
               if (r12 < rcut .and. r13 < rcut) then
                  if (r12 < rcut - buffer) then
                     ...
                     fcut12 = (dcos(pi*(r12 - rcut + buffer)/buffer) + 1._dp)/2._dp
                     dfcut12(1:3) = dsin(pi*(r12 - rcut + buffer)/buffer)/2._dp*pi/buffer*xyz12_red(1:3)
                  end if
                  if (r13 < rcut - buffer) then
                     fcut13 = 1._dp
                     dfcut13(1:3) = 0._dp
                  else
                     fcut13 = (dcos(pi*(r13 - rcut + buffer)/buffer) + 1._dp)/2._dp
                     dfcut13(1:3) = dsin(pi*(r13 - rcut + buffer)/buffer)/2._dp*pi/buffer*xyz13_red(1:3)
                  end if
                  fcut = fcut12*fcut13
...
!             This kernel already contains the prefactor
                     kernel(1:n_sparse) = pref(1:n_sparse)*dexp(-0.5_dp*r(1:n_sparse)**2)
...
!                 For atom 1
                           drdx1(1:n_sparse, l) = drdq(1:n_sparse, 1)*(xyz12_red(l) + xyz13_red(l)) &
                                                  + drdq(1:n_sparse, 2)*2._dp*(r12 - r13)*(xyz12_red(l) - xyz13_red(l))
!                 For atom 2
                           drdx2(1:n_sparse, l) = -drdq(1:n_sparse, 1)*xyz12_red(l) &
                                                  - drdq(1:n_sparse, 2)*2._dp*(r12 - r13)*xyz12_red(l) &
                                                  + drdq(1:n_sparse, 3)*xyz23_red(l)
!                 For atom 3
                           drdx3(1:n_sparse, l) = -drdq(1:n_sparse, 1)*xyz13_red(l) &
                                                  - drdq(1:n_sparse, 2)*2._dp*(r13 - r12)*xyz13_red(l) &
                                                  - drdq(1:n_sparse, 3)*xyz23_red(l)
...
!                virial = virial + dot_product( force3(1:3), xyz13(1:3) )
                        do k1 = 1, 3
                           do k4 = 1, 3
                              virial(k1, k4) = virial(k1, k4) + 0.5_dp*(force2(k1)*xyz12(k4) + force2(k4)*xyz12(k1))
                              virial(k1, k4) = virial(k1, k4) + 0.5_dp*(force3(k1)*xyz13(k4) + force3(k4)*xyz13(k1))
                           end do
                        end do
                     end if
...
!
   function cov_pp(r, d, q) result(cov)
      ...
      if (d == 3 .and. q == 1) then
         do i = 1, size(r)
            if (r(i) >= 1._dp) then
               cov(i) = 0._dp
            else if (r(i) <= 0._dp) then
               cov(i) = 1._dp
            else
               cov(i) = (1._dp - r(i))**(j_int + 1)*((j + 1._dp)*r(i) + 1._dp)
            end if
         end do
      else
   ...
   function cov_pp_der(r, d, q) result(cov_der)
      ...
      if (d == 3 .and. q == 1) then
         do i = 1, size(r)
            if (r(i) >= 1._dp .or. r(i) <= 0._dp) then
               cov_der(i) = 0._dp
            else
!         This expression contains the 1/r factor
               cov_der(i) = (-(j + 1._dp)*(1._dp - r(i))**j_int*((j + 1._dp)*r(i) + 1._dp) + &
                             (1._dp - r(i))**(j_int + 1)*(j + 1._dp))/r(i)
            end if
         end do
      else
         write (*, *) "ERROR: This combination of input parameters is not currently supported for cov_pp!"
