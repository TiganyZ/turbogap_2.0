...
module mc_utils
   ...
   use neighbors_interface, only: get_fractional_coordinates, get_distance

contains

   subroutine initialize_mc_log_file(file_mc_log, do_, n_species, format_string)
      ...
      write_quantities(2) = .true. ! Move
      write_quantities(3) = .true. ! Accepted
      write_quantities(4) = .true. ! E_trial
      write_quantities(5) = .true. ! E_current
      write_quantities(6) = .true. ! E_exp trial
      write_quantities(7) = .true. ! E_exp current
      write_quantities(8) = .true. ! N species
      ...
      quantities(2) = "                Move"
      format_quantities(2) = "                 A16"
      format_length(2) = 16

      quantities(3) = "            Accepted"
      format_quantities(3) = "                  L8"
      format_length(3) = 8

      quantities(4) = "   Energy trial [eV]"
      format_quantities(4) = "               F20.8"
      format_length(4) = 20

      quantities(5) = " Energy current [eV]"
      format_quantities(5) = "               F20.8"
      format_length(5) = 20

      quantities(6) = "    E_exp trial [eV]"
      format_quantities(6) = "               F20.8"
      format_length(6) = 20

      quantities(7) = "  E_exp current [eV]"
      format_quantities(7) = "               F20.8"
      format_length(7) = 20

      quantities(8) = "       N_trial atoms"
      format_quantities(8) = "               A20"
      format_length(8) = 20
      ...
      write (file_mc_log, '(A)') trim(string)

   end subroutine initialize_mc_log_file
...
!    print *, "check_if_atoms_too_close: too_close = ", too_close_out, " minimum_found = ", minimum, " min_dist = ", min_dist
   ...
   subroutine perform_mc_step( &
      ...
      real*8, allocatable, intent(inout) :: positions(:, :), masses(:), local_properties(:, :), &
           ...
           & energies(:), forces(:, :), im_pos(:, :), im_masses(:)
      ...
      character*8, allocatable, intent(inout) :: xyz_species(:), mc_swaps(:), mc_species(:)
      ...
      if (mc_move == "insertion" .or. mc_move == "removal") then
         ...
         if (allocated(energies)) deallocate (energies, positions, &
                                              species, &
                                              xyz_species, fix_atom)
         if (do_forces) then
            deallocate (forces)
            allocate (forces(1:3, 1:n_sites), source=0.0_dp)
         end if

         if (need_velocities) then
            deallocate (velocities)
            allocate (velocities(1:3, 1:n_sites), source=0.0_dp)
         end if
         ...
         if (allocated(local_properties)) then
            ...
            allocate (local_properties(1:n_sites, n_lp), source=0.0_dp)
         end if
      ...
      if (changed%n_sites .or. do_md) then
         ...
         allocate (positions_diff(1:3, 1:n_sites), source=0.0_dp)
         if (allocated(velocities)) deallocate (velocities)
         allocate (velocities(3, n_sites))
      end if
...
!
   ...
   subroutine write_mc_log( &
      file_mc_log, format_mc_log, &
      i_step, move, accepted, &
      energy_current, energy_trial, &
      energy_exp_current, energy_exp_trial, &
      n_sites, n_mu, n_species, species_types &
      )
      integer, intent(in) :: file_mc_log
      character*1024 :: format_mc_log
      integer, intent(in) :: i_step
      character*32, intent(in) :: move
      logical, intent(in) :: accepted
      real(dp), intent(in) :: energy_current
      real(dp), intent(in) :: energy_trial
      real(dp), intent(in) :: energy_exp_current
      real(dp), intent(in) :: energy_exp_trial
      ...
      write (temp_string2, "(I8,1X)") n_sites
      do i = 1, n_mu
         temp_string = ""
         write (temp_string, "(A,1X,I8)") trim(species_types(i)), n_species(i)
         temp_string2 = trim(temp_string2)//" "//trim(temp_string)
      end do

      write (file_mc_log, format_mc_log) &
         i_step, &
         move, &
         accepted, &
         energy_trial, &
